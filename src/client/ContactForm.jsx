...
const ContactForm = () => {
  ...
  const tempErrors = [];
  if (!name.trim()) tempErrors.push('Name input is required.');
  if (!email.trim()) {
    tempErrors.push('Email input is required.');
  } else if (!email.match(emailRegex)) {
    tempErrors.push(`'${email}' is not a valid email.`);
  }
  if (!subject.trim()) tempErrors.push('Subject input is required.');
  if (!message.trim()) tempErrors.push('Message input is required.');
  setErrors(tempErrors);

  if (!tempErrors.length) {
    fetch('https://abdb.w3spaces.com/api/message', { method: 'POST', body, headers })
      .then(res => {
        setStatus(res.status === 201);
        setShowStatus(true);
      })
      .catch(() => {
        setErrors(['Network or server error. Please try again later.']);
        setShowStatus(true);
      });
  }
};
  ...
  };  commented out by AB July 01, 2025 */}
